...
from instacart_etl.validation.schema import _is_type_compatible, validate_column_datatype
...
def test_is_type_compatible(
    ...
    assert (
        _is_type_compatible(actual_type, contract_type)
        is expected_pass
    )
...
def test_validate_single_column_datatype(
    ...
    schema = StructType([
        StructField('test_column', actual_datatype, nullable=True)
    ])
