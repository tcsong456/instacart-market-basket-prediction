...
def test_get_metric_threshold_returns_configured_error_threshold(
    mocker,
):
    ...
    assert result == ThresholdConfig(
        maximum_failed_percent=0.5,
        severity=ValidationSeverity.ERROR,
    )


def test_get_metric_threshold_returns_critical_severity(
    mocker,
):
    metric = ValidationMetric(
        rule_name="order_id.range",
