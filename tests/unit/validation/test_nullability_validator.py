...
def test_nullability_validator_detects_nulls(spark):
    ...
    results = nullability_validator(df, contract=contract)
    results_by_column = {
        result.metadata['column_name']: result
    ...
    assert results_by_column['order_id'].message == "Column 'order_id' is not nullable but contains 2 null value(s)"
    ...
    assert results_by_column['user_id'].message == "Column 'user_id' is not nullable but contains 1 null value(s)"
