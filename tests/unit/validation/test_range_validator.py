...
def test_range_validator_all_values_in_range(spark):
    ...
    assert result.metadata == {"column_name": "value", "minimum": 1, "maximum": 10}
...
def test_range_validator_values_outside_range(spark):
    ...
    invalid_values = {row["value"] for row in result.invalid_rows.collect()}
...
def test_range_validator_with_no_boundary(spark):
    ...
    with pytest.raises(InvalidConstraintError, match="At least one of minimum"):
        validate_range(df, column_name="value", min_value=None, max_value=None)
...
def test_range_validator_limits_invalid_row_sample_to_30(spark):
    df = spark.createDataFrame([(value,) for value in range(50)], ["value"])

    result = validate_range(df, column_name="value", min_value=100, max_value=None)
