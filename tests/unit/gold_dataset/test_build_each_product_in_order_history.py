...
def test_build_each_product_in_order_history(mocker, spark):
    ...
    assert rows[1]["history_order_size"] == "3 3 3 1"
    assert rows[1]["history_reorder_size"] == "0 1 2 1"
    ...
    assert rows[2]["history_order_size"] == "3 3 3 1"
    assert rows[2]["history_reorder_size"] == "0 1 2 1"
    ...
    assert rows[3]["history_order_size"] == "3 3 3 1"
    assert rows[3]["history_reorder_size"] == "0 1 2 1"
    ...
    assert rows[5]["history_order_size"] == "3 3 3 1"
    assert rows[5]["history_reorder_size"] == "0 1 2 1"
    ...
    assert rows[10]["history_order_size"] == "3 3 3 1"
    assert rows[10]["history_reorder_size"] == "0 1 2 1"
    ...
    assert rows[11]["history_order_size"] == "3 3 3 1"
    assert rows[11]["history_reorder_size"] == "0 1 2 1"
...
def test_build_each_product_in_order_history_empty_next_basket(mocker, spark):
    input_df = spark.createDataFrame(
        [
            (1, [[1, 2]], [1, 2], [], "5", "19", "16", "1"),
        ],
...
def test_build_each_product_in_order_history_train_eval_set(mocker, spark):
    input_df = spark.createDataFrame(
        [
            (
                1,
    ...
    assert rows[1]["history_order_size"] == "2 2"
    assert rows[1]["history_reorder_size"] == "0 1"
    assert rows[1]["label"] == -1
    assert rows[1]["is_ordered_history"] == "1 0"
    assert rows[1]["position_in_order_history"] == "1 0"
    assert rows[1]["product_name"] == "apple"

    assert rows[2]["history_order_size"] == "2 2"
    assert rows[2]["history_reorder_size"] == "0 1"
    assert rows[2]["label"] == -1
    assert rows[2]["is_ordered_history"] == "1 1"
    assert rows[2]["position_in_order_history"] == "2 1"
    assert rows[2]["product_name"] == "banana"

    assert rows[3]["history_order_size"] == "2 2"
    assert rows[3]["history_reorder_size"] == "0 1"
    assert rows[3]["label"] == -1
    assert rows[3]["is_ordered_history"] == "0 1"
    assert rows[3]["position_in_order_history"] == "0 2"
    assert rows[3]["product_name"] == "orange"
