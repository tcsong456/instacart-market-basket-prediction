...
def run_product_history_job(
    ...
    product_history = build_each_product_in_order_history(
        df=products, path=data_path, spark=spark
    )
    reorder_history = build_each_reorder_history(reorders)
    ...
    try:
        ...
        write_parquet(
            join_path(output_path, f"product_history_data_{mode}"),
            product_include_none_history,
        )
    finally:
